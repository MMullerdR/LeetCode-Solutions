	...
	for ( i = 0; i < numsSize - 3; i++ ){
		...
		for ( j = i + 1; j < numsSize - 2; j++){
			...
			while ( k < l ){
				...
				if ( soma == (long long) target ){

					int *sum = malloc(sizeof(int) * 4);
					sum[0] = nums[i];
					sum[1] = nums[j];
					sum[2] = nums[k];
					sum[3] = nums[l];
					...
					*returnSize = (*returnSize) + 1;				
					...
					result[(*returnSize) - 1] = sum;

					int temp_numsK = nums[k];
					k++;
					while ( temp_numsK == nums[k] && k < l) {
						k++;
					}

					int temp_numsL = nums[l];
					l--;
					while ( temp_numsL == nums[l] && k < l) {
						l--;
					}
