...
numInvLocais
	...
	retorna o numero de inversos locais
	
====================
*/
int numInvLocais( int *nums, int numsSize ){
...
int contagemGlobal(no_t *raiz, int valor){
	if ( !raiz ){
		return 0;
	}
	...
	if ( raiz->valor > valor ){
		sum = 1 + Tamanho(raiz->dir);
		sum += contagemGlobal(raiz->esq, valor);
	}
	else {
		sum = contagemGlobal(raiz->dir, valor);
	}
...
bool isIdealPermutation( int *nums, int numsSize ){
	...
	for( int i = 0; i < numsSize; i++ ){
		global += contagemGlobal(raiz, nums[i]);
		raiz = Inserir_r(raiz, nums[i]);
	}

	if ( global == numInvLocais(nums, numsSize) ){
		Liberar_r(raiz);
		return true;
	}
